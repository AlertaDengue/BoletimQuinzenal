...
gg_map_risco_classif_dengue <- gg_map_risco_classif(shape = shape_dengue)
```
...
gg_map_risco_classif_chik <- gg_map_risco_classif(shape = shape_chik)
```
...
dif_prop_ano_dengue = round(total_casos_ano_selecionado_dengue/total_casos_ano_anterior_dengue, 2)
```
...
dif_prop_ano_chik = round(total_casos_ano_selecionado_chik/total_casos_ano_anterior_chik, 2)
```
...
dif_prop_ano_total <- round((total_casos_ano_selecionado_dengue + total_casos_ano_selecionado_chik)/(total_casos_ano_anterior_dengue + total_casos_ano_anterior_chik), 2)
...
tabela_1 <- data.frame(
  ...
  var_relacao_ano_anterior = c(dif_prop_ano_chik, dif_prop_ano_dengue, dif_prop_ano_total)
) %>% 
  knitr::kable(caption = "",
               row.names = F,
               col.names = c("Agravo", "Casos suspeitos", "Percentual de casos prováveis ", "Variação em relação ao ano passado"),
               align = c("cccccc"),
...
Semana epidemiológica `r ultima_semana_dengue`: INÍCIO – FIM}
...
Esse ano foram notificados até o momento, `r total_casos` casos suspeitos de dengue, o que corresponde a uma incidência acumulada de `r incidencia_acumulada` casos por 100.000 habitantes. Esse valor corresponde a `r dif_prop_ano` vezes o registrado no ano passado, no mesmo período. Desse total, `r prop_casos_provaveis` são classificados como prováveis. 
A tabela mostra a mesma informação para chikungunya e zika.


\textbf{Tabela 1.} Casos notificados de arboviroses até a semana `r se_max_dengue`
...
Do total de 450 regionais, `r n_regionais_alta_incidencia_dengue` estão indicando alta incidência de suspeição de dengue e `r n_regionais_alta_incidencia_chik` de chikungunya (Figura 4). No Amazonas, na região do Alto Juruá e Triângulo esse indicador aponta aumento. No nordeste, observa-se a 9a região de saúde no Alagoas, 9a na Paraíba, Baturité no CE. No sudeste, Mananciais em SP. 
