...
<!-- Texto humano -->
\begin{itemize}
\item Como era esperado, a notificação decaiu por conta das festividades. É esperado que o país esteja com padrão de crescimento de notificação de dengue de 3 a 10% por semana e chikungunya, 3 a 12% por semana, compatível com a temporalidade das arboviroses. 
\item No final de 2023, já tínhamos RJ, AC, AM e DF acima do limiar epidêmico de dengue. No ano atual, todos essas UFs estão abaixo do limiar epidêmico. Por sua vez, são os estados do sul, RS e SC, que cruzaram o limiar epidêmico precocemente. 
\item Quanto à chikungunya, ao final de 2023, vários estados já estavam acima do limiar epidêmico, com destaque para GO e MG. Esse ano, temos destaque para MT, MS e SP.  PR também com tendência de aumento.
\end{itemize}
...
A curva de casos notificados  de dengue segue mantendo tendência de aumento de casos desde a SE 44 até a SE 51, acima dos canais endêmicos. Houve uma desaceleração na notificação, decorrente das festividades. O número reprodutivo encontra-se estável em Rt = 1,2 nas últimas quatro semanas, indicando crescimento a uma taxa constante. A curva de casos notificados de chikungunya  que já vinha com um leve crescimento desde a SE 45 apresentava tendência de aumento mais acelerado desde a semana 46, com Rt = 1,25. Estima-se que esteja acima do canal endêmico desde a semana 41. 
...
Na região Norte,  apenas o AC se mantem acima do limiar epidêmico, mas com padrão de estabilidade. Os demais estão estáveis ou em queda, mas pode decorrer das festividades. AP, PA e AM estavam acima dos canais endêmicos na semana passada.  

Na região Nordeste, a incidência é menor que 10 por 100.000 hab, em todos os estados. As curvas de incidência estimada estão dentro ou próximas dos canais endêmicos, com a exceção da Bahia, que apresentava (na semana passada) um crescimento da curva de casos mas com incidências relativamente baixas.

No Centro-Oeste, observa-se queda, mas havia tendência de aumento das notificações em MT, GO e DF no boletim anterior.  MT e GO estão com incidência estimada maior do que em 2023 e com risco de crescimento. O DF apresenta incidência menor que em 2023, e crescimento mais lento. Todos encontram-se abaixo do limiar epidêmico. 

Na região Sudeste, é possível observar o crescimento menor das curvas em SP, MG e RJ, mas pode decorrer das festividades. É importante notar que em SP a incidência estimada para o período atual é consideravelmente maior que a observada em 2023. Em RJ e MG, está significativamente menor. Todos estados com incidência acima do canal endêmico, porém abaixo do limiar epidêmico. Os dados do Espírito Santo não estão completos, por isso, não é possível interpretar. 

No Sul, todos os estados apresentam padrão de crescimento, apesar do decréscimo na última semana. Santa Catarina mantém-se acima do limiar epidêmico de 20 casos por 100.000 hab, enquanto Rio Grande do Sul deve manter acima do limiar epidêmico (4 casos por 100.000 hab). Ambos apresentam notificação de casos significativamente maior que em 2023. O Paraná está abaixo do limiar epidêmico do estado, que é mais alto (55 casos por 100.000 hab), e segue com crescimento na incidência estimada, com valores semelhantes a 2023.
...
Em todos os estados, as curvas de incidência de chikungunya seguem abaixo do observado no ano passado, com exceção de MT que indica aumento. A maioria encontra-se dentro do canal endêmico.  Todavia,  alguns estados possuem atividade crescente desse agravo, em particular no Centro-Oeste e Sudeste. No Centro-Oeste, os estados MS e MT têm incidência de chikungunya acima do limiar epidêmico e crescente. No Sudeste, SP e MG observam um aumento mais recente, sendo que SP ultrapassou o limiar epidêmico. 
...
**Norte:** Na semana anterior, quatro estados apresentavam número reprodutivo ligeiramente maior que 1, indicando leve tendência de crescimento de casos (AM, RO, TO e PA). Atualmente encontram-se acima de 1, no AC. Em termos de incidência estimada, os maiores valores são encontrados no AC. 

**Nordeste:** Apenas Bahia mantém uma tendência moderada de aumento de casos. Embora o valor atual seja baixo, notar que foi moderado na semana anterior (Rt = 1,32), sugerindo subnotificação. Todos os estados apresentam baixas incidências estimadas para o momento atual.  

**Centro-Oeste:** Apesar de todos estarem com Rt <1, na semana anterior estavam com número reprodutivo maior que 1. Os maiores valores ocorreram em GO e MT. As maiores incidências ocorrem em GO e DF . A associação de incidência mais alta e tendência de crescimento é indicação de alerta na região.

**Sudeste:** Apenas SP com tendência clara de aumento, Rt = 1,26 na semana anterior e Rt = 1,1 nessa semana. Os demais com ligeiros aumentos. As incidências estimadas mais altas são as de SP e MG, com crescimento observado nas últimas 4 semanas.  ES com dados incompletos. 

**Sul:** Na semana anterior, todos os estados estavam com  número reprodutivo maior que 1, porém de crescimento leve. O maior valor ocorreu no RS (Rt = 1,11). Incidências estimadas em SC e PR altas, mas em SC observa-se uma ligeira queda na última semana. 
...
**Norte:** AP estava com número reprodutivo de 2, mas houve forte queda. Podem ser flutuações numéricas, mas ainda assim, requer atenção. 

**Nordeste:** MA estava com Rt = 1,39, na semana passada e mantem valores acima de 1, indica atenção. Considerando as baixas incidências, não reconhecemos como preocupação no Nordeste. 

**Centro-Oeste:** Observar MT que apresentava Rt = 2 e agora Rt = 1,41, mantendo uma forte aceleração em relação ao observado anteriormente. 

**Sudeste:** Todos os estados com Rt>1, com maiores taxas em SP e MG, com Rt = 1,23 e 1,13, respectivamente. 

**Sul:** PR mantem Rt>1,7, requer atenção.
...
Observa-se que, em termos absolutos, a maior frequência de casos na temporada foi registrada no estado de SP, seguido de PR, SC e GO, um padrão que se mantem nas ultimas semanas (Fig A).  Em termos relativos à população, o maior risco está concentrado no AC, AP, além dos acima mencionados. A Figura C compara a incidência acumulada até o momento com o previsto pelos modelos ensemble. Em quase todos, a incidência está menor ou dentro do previsto. As exceções são:  AP e DF que estão acima do esperado, mas dentro da faixa de incerteza dos modelos.  
