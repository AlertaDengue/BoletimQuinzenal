...
df_ano_selecionado_dengue <- df_dengue %>% 
  filter(as.numeric(substr(SE, 1, 4))  == ano) 
...
df_ano_anterior_dengue <- df_dengue %>% 
  filter(as.numeric(substr(SE, 1, 4)) == (ano - 1))
...
prop_casos_provaveis_total <- paste0(round(((total_casos_provaveis_ano_selecionado_dengue + total_casos_provaveis_ano_selecionado_chik)/(total_casos_ano_selecionado_dengue + total_casos_ano_selecionado_chik)) *100, 2),"%")
...
Esse ano foram notificados até o momento, `r total_casos_ano_selecionado` casos suspeitos de dengue, o que corresponde a uma incidência acumulada de `r incidencia_acumulada_ano_selecionado_dengue` casos por 100.000 habitantes. Esse valor corresponde a `r dif_prop_ano_dengue` vezes o registrado no ano passado, no mesmo período. Desse total, `r prop_casos_provaveis_dengue` são classificados como prováveis. As informações sobre chikungunya também são apresentadas na tabela 1. 

**Tabela 1.** Casos notificados de arboviroses até a semana `r ultima_semana_dengue`

`r tabela_1`

Casos prováveis X Casos Notificados : Incluir frase sobre a figura…

A figura abaixo apresenta a média anual da razão entre os casos prováveis e notificados para todos os estados nos anos entre 2020 e 2024, quanto maior a razão menor o número de casos notificados descartados. Apesar da variabilidade observada nos diferentes anos, alguns estados do $Norte$ como $AC, AM e RO$ tiveram uma taxa de descarte muito maior em 2024 em relação com os outros anos. Ademais, estados como $DF, MG e PR$ que tiveram grandes epidemias em 2024 tiveram uma taxa de descarte menor em relação aos outros anos. 

```{r, fig.align="center", fig.width = 12, fig.height = 3}
# Adicionar figura 1
```

**Figura 1.** Razão entre casos prováveis e notificados por ano e Estado.`

**Casos Acumulados por Estados**

Na figura abaixo estão representadas as incidências de casos notificados acumulados por 100 mil habitantes até a semana `r ultima_semana_dengue`, em comparação com as incidências históricas dos últimos 5 anos .
...
**Figura 2.** Incidência acumulada por 100 mil habitantes sobre os casos notificados até a semana `r ultima_semana_dengue`
...
**Figura 4.** Incidência de casos suspeitos de arboviroses (chikungunya + dengue) para as Unidades da Federação.
