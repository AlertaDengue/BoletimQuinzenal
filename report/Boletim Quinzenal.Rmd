...
df_dengue_chik_estados %>% 
  ...
  gg_timeline_dots()
```
**Figura 1.** Incidência acumulada por 100 mil habitantes sobre os casos notificados de dengue até a semana `r ultima_semana_dengue`.
...
df_dengue_chik_estados %>% 
  ...
  gg_timeline_dots()
```
**Figura 2.** Incidência acumulada por 100 mil habitantes sobre os casos notificados de chikungunya até a semana `r ultima_semana_dengue`.
...
A tabela 2 apresenta um panorama sobre a tendência da incidência de dengue para casos notificados e estimados em todo o país até a semana `r ultima_semana_dengue`. A **tendência** indica se os casos estão crescendo ou decrescendo significativamente.
...
Do total de 450 regionais, `r n_regionais_alta_incidencia_chik` estão indicando alta incidência de suspeição de dengue e `r n_regionais_alta_incidencia_chik` de chikungunya (Figura 5). No Amazonas, na região do Alto Juruá e Triângulo esse indicador aponta aumento. No nordeste, observa-se a $9^a$ região de saúde no Alagoas, $9^a$ na Paraíba, Baturité no CE. No sudeste, Mananciais em SP.
